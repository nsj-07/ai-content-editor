...
<template>
  <div
    class="flex flex-col h-full bg-slate-50/50 rounded-2xl shadow-xl border border-gray-200/80 overflow-hidden bg-grid-pattern relative"
  >
    <!-- Top Action Header -->
    <header
      class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white shadow-xs shrink-0"
    >
      <div class="flex items-center gap-3">
        <div>
          <h1
            class="text-sm font-black text-gray-900 tracking-tight leading-none"
          >
            WordPress Post Publisher
          </h1>
          <span
            class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider"
            >With AI suggestions</span
          >
        </div>
      </div>

      <!-- Settings, Status Badge and Publish Indicators -->
    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      <main class="flex-1 flex flex-col min-w-0 bg-white relative">
        <!-- Post Title Input -->
        <div class="px-8 pt-4 pb-3 border-b border-gray-100 shrink-0">
          <input
            id="post-title-input"
        ...
        <div
          class="flex-1 overflow-y-auto px-8 py-6 cursor-text"
        ...
        >
          ...
          <!-- AI Trigger button in bubble menu -->
          <button
    ...
    <PublishModal :is-open="isPublishOpen" @close="isPublishOpen = false" />
    <ToastNotification />
  </div>
</template>
